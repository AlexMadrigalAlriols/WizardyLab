...
class LeadController extends Controller
{
    ...
    {
        //$this->setMiddleware('lead');
    }

    public function index(Request $request)
    {
        if($request->ajax()) {
            $dataTable = new LeadsDataTable('clients');
            return $dataTable->make();
        }

        $query = Lead::query();
        $total = $query->count();
        $advancedFilters = AdvancedFiltersHelper::getFilters(Lead::class);

        return view('crm.leads.index', compact('total', 'advancedFilters'));
    }
