    ...
    <div class="col-md-8 col-sm-12">
        <div class="mt-1">
            <span class="h2 d-inline-block mt-1">
                <b>{{__('global.edit')}} {{ $client->name }}</b>
            </span>
        </div>
        <form action="{{route('dashboard.clients.update', $client->id)}}" method="POST" class="mt-2 pb-3">
            ...
            <div class="row mt-4">
                <div class="col-md-12 text-end">
                    ...
                    <button class="btn btn-primary ms-2" id="submitBtn"><span class="px-5">{{__('global.save')}} {{__('crud.clients.title_singular')}}</span></button>
                </div>
            </div>
        </form>
    </div>
