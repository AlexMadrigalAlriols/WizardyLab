    ...
    <div class="col-md-6">
        <div class="form-floating mt-3">
            <input type="date" class="form-control" id="birthday_date" name="birthday_date"
                ...
                value="{{ old('birthday_date') ?? $user->birthday_date?->format('Y-m-d') }}">
...
<div class="row">
    ...
    <div class="col-md-6">
        <div class="form-floating mt-3">
            <select class="form-select select2" id="reporting_user_id" name="reporting_user_id"
                aria-label="reporting_user_id">
                <option value="">
                    --</option>
...
<div class="row">
    <div class="col-md-6">
        <div class="form-floating mt-3">
            <select class="form-select select2 @if ($errors->has('attendance_template_id')) is-invalid @endif" id="attendance_template_id"
                name="attendance_template_id" aria-label="Jornada">
                @foreach ($attendanceTemplates as $template)
                    <option value="{{ $template->id }}"
                        {{ $user->attendance_template_id == $template->id || old('attendance_template_id') == $template->id ? 'selected' : '' }}>
                        {{ $template->name }}</option>
                @endforeach
            </select>
            <label for="phone">Jornada <span class="text-danger">*</span></label>
        </div>

        @if ($errors->has('attendance_template_id'))
            <span class="text-danger">{{ $errors->first('attendance_template_id') }}</span>
        @endif
    </div>
</div>
...
<div class="row" siEdit>
    <div class="col-md-6">
        <div class="form-floating mt-3">
            <a href="{{ route('sendResetLink', ['email' => $user->email]) }}" class="btn btn-danger">Send mail reset
                passowrd</a>
        </div>
    </div>
</div>
